...
class FileThread(QtCore.QThread):
	def __init__(self,pathFiles=None,fileName=None,ip="localhost"):
		...
		self.ipDirection = "http://92.222.90.158:12342"
...
if __name__ == '__main__':
	...
	for i in range(filesInPackage):
		allThreads[i] = FileThread(pathFiles="gatitos/",fileName="gatitos"+str(i)+".jpg",ip=ip)
	...
	while True:
		...
		if running == 0: # not threads running
			break


	req = urllib2.Request("http://"+ip+":12342/json")
	req.add_header('Content-Type', 'application/json')
